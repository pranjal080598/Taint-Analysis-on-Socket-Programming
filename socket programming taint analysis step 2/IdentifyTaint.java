...
public class IdentifyTaint {
  public static void main(String[] args) {
    ...
    String linkedlist_content = new String();
    int linkedlist_count=1;
    ...
    try {
      ...
      while (sc.hasNextLine()){
        linkedlist_content = sc.nextLine();
        list.add(linkedlist_content);
      }
      sc.close();
    }
    catch (Exception e) {
    ...
    while (i.hasNext()) {
      System.out.print("Node " + (linkedlist_count++) + " : ");
      ...
      for (String s : content) {
        for (String s1 : port_sanitization_rules){
          if (s.contains(s1)) {
            ...
            System.out.println("\n\nHence, the taint is not propagated to the server using port number" );
